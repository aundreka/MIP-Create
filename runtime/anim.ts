...
/** The CSS `animation` shorthand for a CTA pulse preset (also a CTA's default loop). */
export function ctaPulseAnimation(preset: CtaPulsePreset | undefined): string {
  if (preset === 'custom') return 'none'
  const p = CTA_PULSE[(preset as 'calm' | 'medium' | 'strong') ?? 'medium'] ?? CTA_PULSE.medium
  return `${p.name} ${p.durationMs}ms ease-in-out infinite`
}
...
function loopCss(el: SceneElement, delayMs = 0): string {
  ...
  if (el.type === 'cta') {
    ...
    const p = CTA_PULSE[(el.cta?.pulse as 'calm' | 'medium' | 'strong') ?? 'medium'] ?? CTA_PULSE.medium
    return `${p.name} ${p.durationMs}ms ease-in-out ${delayMs}ms infinite`
  }
  return ''
}
