...
The same element also detects the **viewer's device**: the `{device}` token (alias `{os}`) renders **iOS**, **Android**, **Windows**, **Mac** or **Linux**, read once from the browser at load (`runtime/elements/device.ts` — Chromium's `userAgentData.platform` client hint first, then the user-agent string and `navigator.platform`; an iPad that presents as a Mac is unmasked by its touch screen). It works everywhere the shared formatter does, so "Get it on {device}" is a legal dynamic-date format, header band or ring label. Nothing matched renders empty. `setDeviceOverride()` lets the editor force a platform for preview, like the preview date.
