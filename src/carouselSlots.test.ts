...
import { assignCarouselSlot, carouselCandidates, carouselLabelFor, carouselMembers, carouselOptionLabel, carouselSlotSummary, setCarouselCanvasVisible } from './carouselSlots'
...
describe('carousel label assignment', () => {
  it('fills an empty slot with one patch', () => {
    const a = el('a')
    const edits = assignCarouselSlot({ nextId: 'a', current: undefined, gameId: GAME, choice: 2, elements: [a] })
  ...
  it('frees the previous holder so a slot is never double-booked', () => {
    ...
    const b = el('b')
    const edits = assignCarouselSlot({ nextId: 'b', current: a, gameId: GAME, choice: 1, elements: [a, b] })
  ...
  it('moves an element that already labels another choice instead of cloning it', () => {
    const a = el('a', { carouselRole: { gameId: GAME, role: 'label', choice: 1 } })
    const edits = assignCarouselSlot({ nextId: 'a', current: undefined, gameId: GAME, choice: 3, elements: [a] })
    expect(edits).toHaveLength(1)
  ...
  it('releases a slot when given an empty id', () => {
    ...
    const edits = assignCarouselSlot({ nextId: '', current: a, gameId: GAME, choice: 2, elements: [a] })
    expect(edits).toEqual([{ id: 'a', patch: { carouselRole: undefined } }])
  })

  it('does nothing when the element is already in that slot', () => {
    const a = el('a', { carouselRole: { gameId: GAME, role: 'label', choice: 1 } })
    expect(assignCarouselSlot({ nextId: 'a', current: a, gameId: GAME, choice: 1, elements: [a] })).toEqual([])
  })
  ...
  it('keeps a label’s canvas-visibility flag when it moves slot', () => {
    ...
    const edits = assignCarouselSlot({ nextId: 'a', current: undefined, gameId: GAME, choice: 4, elements: [a] })
  ...
  it('is exclusive with the other board roles — an element does one job', () => {
    ...
    const edits = assignCarouselSlot({ nextId: 'a', current: undefined, gameId: GAME, choice: 1, elements: [a] })
  ...
  it('claims labels tagged with no game named, as a single-game scene produces', () => {
    ...
    expect(carouselLabelFor([a, b], GAME, 1)?.id).toBe('a')
  })
